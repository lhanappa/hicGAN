...
def hicGAN_g(t_image, is_train=False, reuse=False):
    ...
    with tf.variable_scope("hicGAN_g", reuse=reuse) as vs:
        ...
        n = Conv2d(n, 64, (3, 3), (1, 1), act=tf.nn.relu,
                   padding='SAME', W_init=w_init, name='n64s1/c')
        temp = n
        # B residual blocks
        for i in range(5):
            nn = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME',
                        W_init=w_init, b_init=b_init, name='n64s1/c1/%s' % i)
            nn = BatchNormLayer(nn, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='n64s1/b1/%s' % i)
            nn = Conv2d(nn, 64, (3, 3), (1, 1), act=None, padding='SAME',
                        W_init=w_init, b_init=b_init, name='n64s1/c2/%s' % i)
            nn = BatchNormLayer(nn, is_train=is_train,
                                gamma_init=g_init, name='n64s1/b2/%s' % i)
            nn = ElementwiseLayer(
                [n, nn], tf.add, name='b_residual_add/%s' % i)
            n = nn
        n = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s1/c/m')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s1/b/m')
        n = ElementwiseLayer([n, temp], tf.add, name='add3')
        # B residual blacks end. output shape: (None,w,h,64)
        n = Conv2d(n, 128, (3, 3), (1, 1), act=None,
                   padding='SAME', W_init=w_init, name='n128s1/1')
        n = Conv2d(n, 256, (3, 3), (1, 1), act=None,
                   padding='SAME', W_init=w_init, name='n256s1/2')
        n = Conv2d(n, 1, (1, 1), (1, 1), act=tf.nn.tanh,
...
def hicGAN_d(t_image, is_train=False, reuse=False):
    ...
    with tf.variable_scope("hicGAN_d", reuse=reuse) as vs:
        n = InputLayer(t_image, name='in')
        n = Conv2d(n, 64, (3, 3), (1, 1), act=lrelu,
                   padding='SAME', W_init=w_init, name='n64s1/c')

        n = Conv2d(n, 64, (3, 3), (2, 2), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s2/c1')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s2/b1')
        # output shape: (None,w/2,h/2,64)
        n = Conv2d(n, 64, (3, 3), (1, 1), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s1/c2')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s1/b2')

        n = Conv2d(n, 64, (3, 3), (2, 2), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s2/c3')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s2/b3')
        # output shape: (None,w/4,h/4,64)
        n = Conv2d(n, 64, (3, 3), (1, 1), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s1/c4')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s1/b4')

        n = Conv2d(n, 64, (3, 3), (2, 2), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n64s2/c5')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n64s2/b5')
        # output shape: (None,w/8,h/8,256)
        n = Conv2d(n, 128, (3, 3), (1, 1), act=lrelu, padding='SAME',
                   W_init=w_init, b_init=b_init, name='n128s1/c')
        n = BatchNormLayer(n, is_train=is_train,
                           gamma_init=g_init, name='n128s1/b')
...
def train(train_dir, valid_dir, model_dir, num_epochs=5):
    ...
    t_image = tf.placeholder(
        'float32', [None, 40, 40, 1], name='input_to_generator')
    t_target_image = tf.placeholder(
        'float32', [None, 40, 40, 1], name='t_target_hic_image')
    ...
    summary_writer = tf.summary.FileWriter(
        '%s' % graph_dir, graph=tf.get_default_graph())
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        for idx in range(0, len(hr_mats_train)-batch_size, batch_size):
            ...
            errD, _ = sess.run([d_loss, d_optim], {
                            t_image: b_imgs_input, t_target_image: b_imgs_target})
            # update G
        ...
        if mse_val < best_mse_val:
            ...
            tl.files.save_npz(net_g.all_params, name=checkpoint +
                            '/g_{}_best.npz'.format(tl.global_flag['mode']), sess=sess)
            tl.files.save_npz(net_d.all_params, name=checkpoint +
                            '/d_{}_best.npz'.format(tl.global_flag['mode']), sess=sess)
        else:
            wait += 1
            if wait >= patience:
        ...
        # record variables for TensorBoard visualization
        summary = sess.run(
            merged_summary, {t_image: b_imgs_input, t_target_image: b_imgs_target})
        summary_writer.add_summary(summary, epoch)

        # save model every 5 epochs
        if (epoch <= 5) or ((epoch != 0) and (epoch % 5 == 0)):
            tl.files.save_npz(net_g.all_params, name=checkpoint +
                            '/g_{}_{}.npz'.format(tl.global_flag['mode'], epoch), sess=sess)
            tl.files.save_npz(net_d.all_params, name=checkpoint +
                            '/d_{}_{}.npz'.format(tl.global_flag['mode'], epoch), sess=sess)
